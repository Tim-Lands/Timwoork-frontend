  ...
  return (
    <>
      <div className="timlands-panel">
        <Posts status="pending" postsList={postsList} onChangePostsList={setPostsList} />
...
export async function getServerSideProps(ctx) {
  ...
  if (!pageNumber)
    return {
      redirect: {
        permanent: false,
        destination: "/tw-admin/posts/pendings?pageNumber=1"
      }
    }
  const token = cookies(ctx).token_dash || ""
  const params = { page: pageNumber, status: 2, like: search && `title,${search}` }
  try {
    console.log(token)
