...
function Id({ query }) {
    ...
    const cancelAmount = async () => {
        setIsLoading(true)
        try {
            const res: any = await API.post(`dashboard/withdrawals/${query.id}/cancel`, { cause: cause }, {
                headers: { Authorization: `Bearer ${token}` }
            })
            if (res.status === 200) {
                router.push('/tw-admin/withdrawables')
                    ...
                    <div className="timlands-modal-form">
                        <Spin spinning={isLoading}>
                            <div className="timlands-form">
                                <label className="label-block" htmlFor="input-title">أذكر سبب الرفض</label>
                                <textarea
                                    id="input-title"
                                    name="title"
                            ...
                            <div className="py-2">
                                <button className='btn butt-primary butt-sm mx-1' onClick={cancelAmount}>رفض</button>
                                <button className='btn butt-red butt-sm mx-1' onClick={() => setIsShowCause(false)}>إغلاق</button>
                            </div>
                        </Spin>
                    </div>
                }
                        ...
                        <table className="table">
                            <tbody>
                                ...
                                <tr>
                                    ...
                                    <td>{getData && getData.data.withdrawalable.profile.withdrawable_amount}</td>
                                </tr>
                                <tr>
                                    <th>مستواه: </th>
                                    <td>{getData && getData.data.withdrawalable.profile.level && getData.data.withdrawalable.profile.level.name_ar}</td>
                                </tr>
                                <tr>
                                    <th>تاريخ الميلاد: </th>
                                    <td>{getData && getData.data.withdrawalable.profile.date_of_birth} </td>
                                </tr>
                            </tbody>
                        </table>
                    </div>
