...
function linkify(text, id) {
  ...
  const before = text?.replace(urlRegex, function (url) {
    ...
    if (newUrl.startsWith("https://timwoork")) {
      ...
      return (
        '<a rel="noreferrer" href="/redirect/f?url=' +
        newUrl +
        `&*conversations/${id}` +
        '" target="_blank">' +
        url +
...
const SingleConversation = ({ id }: { id: number }) => {
          ...
          <div className="timlands-form">
            ...
            <div className="py-1 d-flex">
              ...
              >
                ...
                <option value="2">{getAll("Cancellation_reason")}</option>
              </select>
              <button
                type="button"
                style={{ width: "65%" }}
            ...
            >
              <input
                ...
                className={
                  "timlands-inputs " +
                  (messageErrors && messageErrors.message && " has-error")
                }
                disabled={sendMessageLoading}
                autoComplete="off"
                value={message}
