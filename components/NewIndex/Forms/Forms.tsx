...
export function FormInput({ name, value, title, handleChange, validationsErrors, isLoading, type = 'text', isVisibleBtn = false, setIsShowenPass, isShowenPass }: any): ReactElement {
    return (
        <div className="timlands-form" style={{ position: 'relative' }}>
            ...
            />
            {isVisibleBtn && 
                <button className={`form-toggle-password ${isShowenPass ? 'is-active' : ''}`} type='button' onClick={() => setIsShowenPass(!isShowenPass)}>
                    <span className="material-icons material-icons-outlined">{isShowenPass ? 'visibility_off' : 'visibility'}</span>
                </button>
            }
            {validationsErrors &&
                <div style={{ overflow: 'hidden' }}>
                    <motion.div initial={{ y: -12, opacity: 0 }} animate={{ y: 0, opacity: 1 }} className="timlands-form-note form-note-error">
    ...
    return (
        <div className="timlands-form">
            ...
            <select
                ...
                className={"timlands-inputs select " + (props.validationsErrors && ' has-error')}
                onChange={props.handleChange}
                value={props.value}
            >
                <option value=''>{props.title}</option>
                {props.data && props.data.map((item: any, i: number) => (
                    <option key={i} value={item.id}>{(lang == 'ar') ? item.name_ar : item.name_en}</option>
                ))}
            </select>
