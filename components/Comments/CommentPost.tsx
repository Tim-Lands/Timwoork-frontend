...
function CommentPost(props: any): ReactElement {
  ...
  return (
    <li className="comment-item">
      {!props.reply && props.canReply && (
        <button
          className="btn butt-light butt-xs flex-center reply-button"
          onClick={() => setIsShowCommentForm(!isShowCommentForm)}
        >
          <span
            className="material-icons-outlined outline-star"
            style={{ fontSize: 16 }}
          >
            reply
          </span>{" "}
                ...
                <div className="timlands-form">
                  ...
                  {validationsErrors && validationsErrors.comment && (
                    <div style={{ overflow: "hidden" }}>
                      ...
                      >
                        <p className="text">{validationsErrors.comment[0]}</p>
                      </motion.div>
                    </div>
                  )}
                </div>
      ...
      <div className="d-flex">
        ...
        <div className="comment-item-content">
          ...
          <p className="item-text">{props.content}</p>
        </div>
      </div>
      {props.replies && (
          ...
          {props.replies.map((e: any) => (
            <li key={e.id} className="reply-item">
              <div className="d-flex">
                <div className="comment-item-avatar">
                  <img
                    src={e.avatar || "/avatar.png"}
                    className="circular-img md-size"
                    alt=""
                  />
                </div>
                <div className="comment-item-content">
                  ...
                  <p className="item-text">{e.content}</p>
                </div>
              </div>
            </li>
          ))}
